...
```{r}
accidents_bmr_ft_xy <- accidents_bmr_ft %>%
  mutate(X = accidents_xy[, 1], Y = accidents_xy[, 2])


mean_centers_by_year <-  accidents_bmr_ft_xy %>%
  group_by(year) %>%
  summarize(
    mean_x = mean(X),
    mean_y = mean(Y)
...
median_centers_by_year <- accidents_bmr_ft_xy %>%
  group_by(year) %>%
  summarize(
    median_x = median(X),
    median_y = median(Y)
